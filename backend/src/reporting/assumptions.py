...
class AssumptionEntry:
    """Single explicit assumption used by the platform."""
...
class AssumptionRegistry:
    """Collection of active, blocked, and review assumptions."""

    entries: list[AssumptionEntry]


def _build_registry() -> AssumptionRegistry:
    """Build the current assumption registry from code and settled project rules."""

    config = load_yaml_config("src/config/legislative.yaml")
    positive_result_coefficient = float(config["pe"]["positive_result_coefficient"])
    historical_average_coefficient = float(config["pe"]["historical_average_coefficient"])

    entries = [
        AssumptionEntry(
            assumption_name="ppna_day_count_basis",
        ...
        AssumptionEntry(
            assumption_name="ppna_negative_and_zero_premiums_retained",
            current_value=True,
            source_type="MENTOR",
        ...
        AssumptionEntry(
            assumption_name="pe_positive_result_coefficient",
            current_value=positive_result_coefficient,
            source_type="FACT",
        ...
        AssumptionEntry(
            assumption_name="pe_historical_average_coefficient",
            current_value=historical_average_coefficient,
            source_type="FACT",
        ...
        AssumptionEntry(
            assumption_name="pe_ignores_contract_year_count_for_actuarial_logic",
            current_value=True,
            source_type="MENTOR",
        ...
        AssumptionEntry(
            assumption_name="sap_formula_uses_declaration_notification_status_and_paid_amount",
            current_value=(
                'if Date de clôture < "Date de déclaration" then 0; if "Date de déclaration" < Date de clôture < '
                '"Date de notification reglement /REJET" then "Montant sinistre déclaré"; else REJET=>0, '
                'SAP=>"Montant sinistre déclaré", REGLE=>max(0,"Montant sinistre déclaré"-"Montant réglé")'
            ),
            source_type="WORKBOOK_FORMULA",
        ...
        AssumptionEntry(
            assumption_name="sap_blank_notification_falls_back_to_status_branch",
            current_value=True,
            source_type="WORKBOOK_FORMULA",
        ...
        AssumptionEntry(
            assumption_name="ppna_output_sheet_non_authoritative_template",
            current_value=True,
        ...
        AssumptionEntry(
            assumption_name="ibnr_method_chain_ladder_volume_weighted",
        ...
        AssumptionEntry(
            assumption_name="ibnr_occurrence_year_window",
        ...
        AssumptionEntry(
            assumption_name="ibnr_closing_year",
            current_value=2025,
            source_type="CONFIG",
        ...
        AssumptionEntry(
            assumption_name="ibnr_product_segmentation_mode",
            ...
            status="active",
            justification=(
                "Two modes are available and user-selectable via the UI / legislative.yaml: "
                "(1) mixed — one triangle over all ADE products, matches workbook reference; "
                "(2) homogeneous_by_product — one independent triangle per product (IMMO, CONSO, WARDA, AC-ELITE), "
                "satisfying the actuarial rule of not mixing heterogeneous products. "
                "Reconciliation reference uses mixed mode. Set segment_by_product: true in legislative.yaml for homogeneous mode."
            ),
            affected_modules=["ibnr", "assumptions", "audit_run"],
        ),
        AssumptionEntry(
            assumption_name="ibnr_product_label_normalization",
        ...
        AssumptionEntry(
            assumption_name="ibnr_mack_variance_process_only",
        ...
        AssumptionEntry(
            assumption_name="ibnr_mack_se_correlation_ignored",
        ...
        AssumptionEntry(
            assumption_name="ibnr_bf_prior_basis_mature_ratio",
        ...
        AssumptionEntry(
            assumption_name="ibnr_benktander_k2",
        ...
        AssumptionEntry(
            assumption_name="ibnr_bootstrap_odp_residual",
    ...
    return AssumptionRegistry(entries=entries)


def _write_markdown(registry: AssumptionRegistry, markdown_path: Path) -> None:
    """Write the assumption registry as Markdown."""

    lines = ["# Assumption Registry", ""]
    for entry in registry.entries:
        lines.append(f"## {entry.assumption_name}")
        lines.append(f"- current_value: `{entry.current_value}`")
        lines.append(f"- source_type: `{entry.source_type}`")
        lines.append(f"- source_reference: `{entry.source_reference}`")
        lines.append(f"- status: `{entry.status}`")
        lines.append(f"- justification: {entry.justification}")
        lines.append(f"- affected_modules: `{entry.affected_modules}`")
        lines.append("")
    markdown_path.write_text("\n".join(lines), encoding="utf-8")


def generate_assumption_registry(output_dir: Path) -> dict[str, object]:
    """Generate machine-readable and human-readable assumption registry artifacts."""

    try:
        output_dir.mkdir(parents=True, exist_ok=True)
        registry = _build_registry()
        json_path = output_dir / "assumption_registry.json"
        markdown_path = output_dir / "assumption_registry.md"
        payload = {
            "entries": [asdict(entry) for entry in registry.entries],
            "status_counts": {
                "active": sum(1 for entry in registry.entries if entry.status == "active"),
                "blocked": sum(1 for entry in registry.entries if entry.status == "blocked"),
                "needs_review": sum(1 for entry in registry.entries if entry.status == "needs_review"),
            },
        }
        json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False, sort_keys=True), encoding="utf-8")
        _write_markdown(registry, markdown_path)
        LOGGER.info("Wrote assumption registry to %s and %s", json_path, markdown_path)
        return {"json_path": str(json_path), "markdown_path": str(markdown_path), "payload": payload}
    except Exception as exc:
        LOGGER.error("Failed to generate assumption registry: %s", exc)
        raise
