...
import { resetFrontendState } from "@/lib/resetFrontendState";
...
export function Topbar({ title, subtitle }: { title: string; subtitle?: string }) {
  ...
  const onResetFrontend = () => {
    const accepted = window.confirm(
      "Réinitialiser toutes les valeurs frontend ? Cette action efface les tokens, le cache et recharge l'application.",
    );
    if (!accepted) return;

    resetFrontendState(queryClient);
    ...
    <header className="sticky top-0 z-30 bg-card/95 backdrop-blur-md border-b border-border">
      <div className="flex items-center gap-4 px-6 lg:px-8 h-[4.5rem]">
        ...
        <div className="flex-1 min-w-0">
          ...
          {subtitle && (
            <p className="text-[11px] text-muted-foreground truncate mt-0.5 tracking-wide">
              {subtitle}
            </p>
          )}
        </div>
                ...
                <div className="p-1.5">
                  ...
                  <div className="mt-1.5 border-t border-border pt-1.5">
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left transition-all hover:bg-destructive/10 text-destructive"
                    >
                      <RotateCcw className="h-3.5 w-3.5" />
                      <span className="text-[12px] font-semibold">Reset frontend values</span>
                    </button>
                  </div>
                </div>
              </div>
